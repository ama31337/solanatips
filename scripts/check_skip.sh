...
then
	echo "`date` ALARM! skiprate is above average"
	 "${SCRIPT_DIR}/../Send_msg_toTelBot.sh" "$HOSTNAME inform you:" "ALARM! skiprate= ${SKIP_PECENT}% is above average= ${SKIP_PECENT_TOTAL}%"  2>&1 > /dev/null
else 
	echo "`date` skiprate is below average"
fi


if (( $(echo "${SKIP_PECENT} > ${SKIP_LIMIT}" | bc -l) ))
then
    echo "`date` ALARM! skiprate is above limit"
     "${SCRIPT_DIR}/../Send_msg_toTelBot.sh" "$HOSTNAME inform you:" "ALARM! skiprate= ${SKIP_PECENT}% is above limit= ${SKIP_LIMIT}%"  2>&1 > /dev/null
else
    echo "`date` skiprate is below limit"
fi
