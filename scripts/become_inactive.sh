...
REMOTE_LEDGER=${REMOTE_LEDGER_PATH}
...
if [ $? == 0 ];then
   echo "start script"
   ${BIN_PATH}/solana-validator -l ${LOCAL_LEDGER} wait-for-restart-window --min-idle-time 2 --skip-new-snapshot-check
   ${BIN_PATH}/solana-validator -l ${LOCAL_LEDGER} set-identity ${HOME}/keys/unstaked-identity.json
